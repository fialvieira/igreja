...
class Profissao {
  ...
  public function __construct($id = null) {
    if (!is_null($id)) {
      ...
      $c = My::con();
      $r = $c->query("CALL profissao_seleciona($id)");
      $l = $r->fetch_assoc();
      if ($l) { 
        $this->id = $id;
        $this->nome = $l["nome"];
  ...
  */
  ...
  public function salva() {
    ...
    if ($this->id) {
      $com = $c->prepare("CALL profissao_altera(?,?,?,?,?,?,?)");
      $com->bind_param(
              "issiiss",
              $this->id,
              $this->nome,
      ...
      $com = $c->prepare("CALL profissao_insere(?,?,?,?,?,?)");
      $com->bind_param(
              "ssiiss", 
              $this->nome,
              $this->descricao,
  ...
  public static function seleciona($valor = null) {
    ...
    if($valor == "" || !isset($valor) || is_null($valor)){
      $r = $c->query("CALL profissoes_seleciona()");
    }else{
      $r = $c->query("CALL profissoes_fulltext_seleciona($valor,EMPRESA)");
    }
    $retorno = [];
