...
class TipoBiblioteca {
  ...
  public function __construct($id = null) {
    if (!is_null($id)) {
      ...
      $c = My::con();
      $r = $c->query("CALL tipo_biblioteca_seleciona($id)");
      $l = $r->fetch_assoc();
      if ($l) { 
        $this->id = $id;
        $this->nome = $l["nome"];
  ...
  */
  ...
  public function salva() {
    ...
    if ($this->id) {
      $com = $c->prepare("CALL tipo_biblioteca_altera(?,?,?,?,?,?)");
      $com->bind_param(
              "isiiss",
              $this->id,
              $this->nome,
      ...
      $com = $c->prepare("CALL tipo_biblioteca_insere(?,?,?,?,?)");
      $com->bind_param(
              "siiss", 
              $this->nome,
              $this->empresa_id,
  ...
  public static function seleciona($valor = null) {
    ...
    if($valor == "" || !isset($valor) || is_null($valor)){
      $r = $c->query("CALL tipo_biblioteca_seleciona(EMPRESA)");
    }else{
      $r = $c->query("CALL tipo_biblioteca_fulltext_seleciona($valor,EMPRESA)");
    }
    $retorno = [];
