<table>
    ...
    <?php foreach ($retorno as $ret): ?>
        <tr>
            ...
            <td class="acoes">
                <div>
                    <?php if (Aut::temPermissao(Aut::$modulos['TRANSFERENCIA_PLANO_DE_CONTAS'], \modelo\Permissao::REWRITE)) : ?>
                        <!--<a id="a_acao" title="Alterar dados" class="alterar"
                           href="movimentacao_financeira.php?codigo=<?/*= $ret["id"] */?>"></a>-->
                    <?php endif; ?>
                    <?php if (Aut::temPermissao(Aut::$modulos['TRANSFERENCIA_PLANO_DE_CONTAS'], \modelo\Permissao::DEL)) : ?>
                        <a class="excluir"
