    ...
    try {
        ...
        ajax('pesquisa.php', strpar, function (html) {
            try {
                ...
                hash.quorum = document.getElementById('quorum').value;
                hash.local = document.getElementById('local').value;
                historico.push(hash);
            } catch (e) {
                alerta.erro(e, 5);
            }
...
historico.load(function (hash) {
    if (hash.status || hash.quorum || hash.local) {
        ...
        document.getElementById('quorum').value = hash.quorum;
        document.getElementById('local').value = hash.local;
        pesquisa();
    }
    if (hash.texto) {
