...
DROP PROCEDURE IF EXISTS `relatorio_conselho_diretor_rec`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `relatorio_conselho_diretor_rec`(
	vempresa	INT(11),
	vano 			INT,
	vmes 			INT,
	vtipo			CHAR(1),
	vmae_id		INT(11) UNSIGNED
)
BEGIN
	SELECT tbl.*
	FROM (
