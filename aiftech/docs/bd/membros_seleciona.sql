...
DROP PROCEDURE IF EXISTS `membros_seleciona`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `membros_seleciona`(
	vempresa INT(11),
	vativo	 CHAR(1),
	vlimit   INT,
	vorder   VARCHAR(100)
...
--		ORDER BY T1.modified DESC, T1.nome ASC
		ORDER BY 
					CASE WHEN vorder = 'modified' THEN T1.modified ELSE '' END DESC, T1.nome  
		LIMIT vlimit;
